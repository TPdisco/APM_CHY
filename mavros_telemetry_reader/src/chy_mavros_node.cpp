...
#include "mavros/mavros_router.hpp" //（负责 MAVLink 串口/UDP 多路复用）
#include "mavros/mavros_uas.hpp" //（飞控状态机、坐标系、插件装载器）
...
int main(int argc, char * argv[])
{
  ...
  //MultiThreadedExecutor 允许 2 个线程并行执行回调（一个给 Router，一个给 UAS）
  ...
  //它把来自飞控的 mavlink 消息转发给 UAS，把 UAS 的指令转发给飞控/地面站
